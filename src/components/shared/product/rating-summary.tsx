...
export default function RatingSummary({
  ...
  const safeRatingDistribution = Array.isArray(ratingDistribution)
    ? ratingDistribution
    : [];
    ...
    return (
      <>
        ...
        <div className="space-y-3">
          {safeRatingDistribution
            .sort((a, b) => b.rating - a.rating)
            .map(({ rating, count }) => (
              <div key={rating} className="flex gap-4 items-center">
                <div className="flex items-center gap-1">
                  <Rating rating={rating} />
                </div>

                <Progress
                  value={Math.round((count / numReviews) * 100)}
                  className="h-[6px]"
                />
      ...
      <Popover>
        ...
        <PopoverContent className="w-auto p-4" align="end">
          <div className="flex flex-col gap-2">
            ...
            <Separator />
            <Link
              className="highlight-link text-center text-[14px]"
              href="#reviews"
            >
