...
export default function ProductLinkParser({
  content,
  className = "",
}: ProductLinkParserProps) {
      ...
      try {
        ...
        if (data.success && data.matches.length > 0) {
          ...
          data.matches.forEach((match: ProductMatch) => {
            ...
            const matchingEntries = slugsAndText.filter(
              (item) => item.slug === match.slug
            );
