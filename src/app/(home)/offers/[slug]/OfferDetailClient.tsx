...
export default function OfferDetailClient({ offerId }: Props) {
  ...
  const addToCartMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result?.success !== false) {
        toast.success(
          `${selectedSlots} slot${selectedSlots > 1 ? "s" : ""} added to cart!`
        );
  ...
  const formatTimeLeft = (endDate: string) => {
    ...
    if (totalHours <= 2)
      urgency = "critical"; // Red for 2 hours or less
  ...
  const datas = [
    {
      ...
      title: "Fast Delivery",
      description:
        "Get your slots delivered at your doorstep in 3 hours or less.",
    },
    {
      id: 2,
      icon: <Security />,
      ...
      <Container>
        <div className="text-center py-10">
          <p className="text-red-500">
            Failed to load offer. Please try again.
          </p>
      ...
      <section>
        <Container>
          {isLoading ? (
            ...
            offer && (
              <div
                className={`py-4 grid grid-cols-1 md:grid-cols-6 lg:grid-cols-8 gap-8 bg-white my-6 p-3 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              >
                {/* Offer Image Gallery */}
                <div
                  className={`col-span-3 transition-all duration-500 delay-100 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-8"}`}
                >
                  <div className="relative w-full h-96 lg:h-[500px] rounded-lg overflow-hidden bg-gray-100">
                    ...
                    {offer.available_slots === 0 && (
                      <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                        <Badge variant="destructive" className="text-xl p-2">
                          SOLD OUT
                        </Badge>
                      </div>
                    )}
                ...
                {/* Offer Details */}
                <div
                  className={`col-span-3 transition-all duration-500 delay-200 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
                >
                  <h1 className="text-2xl font-bold mb-2 transition-all duration-300 hover:text-green-600">
                    {offer.title}
                  </h1>
                      ...
                      <div className="flex gap-2 text-center">
                        {parseInt(timeLeft.days) > 0 && (
                          <div className="rounded-md border bg-white px-3 py-2">
                            <div className="text-xl font-semibold font-mono text-gray-900">
                              {timeLeft.days}
                            </div>
                  ...
                  {offer.end_date && timeLeft?.expired && (
                    <div className="mb-4">
                      <Badge
                        variant="destructive"
                        className="text-lg p-2 animate-bounce"
                      >
                  ...
                  <p className="text-[12px] pt-2 mb-4">
                    {offer.available_slots} of {offer.total_slots} slots
                    available
                  </p>
                    ...
                    <div className="flex items-center text-gray-600">
                      ...
                      <div>
                        <div className="font-medium">
                          {offer.available_slots} of {offer.total_slots}
                        </div>
                  ...
                  <div className="mb-4">
                    <div className="flex justify-between text-sm text-gray-600 mb-2">
                      ...
                      <span>
                        {Math.round(
                          ((offer.total_slots - offer.available_slots) /
                            offer.total_slots) *
                            100
                        )}
                        % filled
                ...
                {/* Add to Cart Section */}
                <div
                  className={`col-span-1 md:col-span-6 lg:col-span-2 border border-[#DDD5DD] px-4 py-2 w-full h-fit transition-all duration-500 delay-300 hover:shadow-lg hover:border-green-200 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}
                >
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setSelectedSlots(
                            Math.min(offer.available_slots, selectedSlots + 1)
                          )
                        }
                  ...
                  <div className="border-t pt-4 mb-4 transition-all duration-300">
                    ...
                    <p className="text-sm text-gray-600 mt-1 transition-all duration-300">
                      ...
                      {formatNaira(offer.price_per_slot)}
                    </p>
                  </div>
                  ...
                  >
                    {isAddingToCart || addToCartMutation.isPending ? (
                      ...
                      <>
                        ...
                        Add {selectedSlots} Slot{selectedSlots > 1 ? "s" : ""}{" "}
                        to Cart
                      </>
                    )}
                  </Button>
          ...
          {offer?.description && (
            <div
              className={`bg-white my-6 p-3 transition-all duration-500 delay-400 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
            >
              <Accordion type="single" collapsible className="w-full">
                <AccordionItem value="item-1" className="!border-none">
                  <AccordionTrigger className="hover:text-green-600 transition-colors duration-200">
                    Offer Description
                  </AccordionTrigger>
