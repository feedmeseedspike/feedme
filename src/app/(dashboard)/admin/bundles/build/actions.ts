...
export async function createBundleAction(bundleData: {
  name: string;
  price: number;
  description?: string;
  ...
  if (bundleError) {
    console.error('[ERROR] Bundle insert error:', bundleError);
  ...
  if (bundleData.productIds.length > 0 && bundleRecord) {
    ...
    if (bundleProductsError) {
      ...
      console.error('[ERROR] Attempted to insert data:', bundleProductsData);
