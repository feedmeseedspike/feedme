    ...
    if (bundleId) {
      ...
      if (existingOfferItem) {
        ...
        if (newTotalQuantity > offerData.available_slots) {
          throw new Error(`Only ${offerData.available_slots} slots available for "${offerData.title}". You currently have ${existingOfferItem.quantity} in cart.`);
        }
      ...
      return { success: false, error: "No product, bundle, or offer ID provided." };
    }
...
export async function updateCartItemQuantity(cartItemId: string, quantity: number): Promise<UpdateCartItemQuantitySuccess | UpdateCartItemQuantityFailure> {
  ...
  try {
    ...
    if (cartItem.offer_id) {
      ...
      if (quantity > offerData.available_slots) {
        throw new Error(`Only ${offerData.available_slots} slots available for "${offerData.title}"`);
      }
    }
