  ...
  try {
    ...
    const userHtml = `
      <div style="font-family: Arial, sans-serif; background: #fff; max-width: 600px; margin: 0 auto; border: 1px solid #eaeaea; border-radius: 8px; padding: 24px;">
        ...
        <div style="text-align: center; margin-top: 20px; padding-top: 10px; border-top: 1px dashed #ddd;">
             <a href="${process.env.NEXT_PUBLIC_SITE_URL}/order/track?id=${orderNumber}" 
                style="background-color: #f3f3f3; color: #333; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-size: 14px; display: inline-block;">
                Track My Order
             </a>
        </div>
    ...
    console.log(`📤 Sending status update email (${newStatus}) to ${userEmail}...`);
