...
async function testJollofRiceDetailed() {
  try {
    console.log("Testing Jollof Rice query with detailed RAG analysis...");
    ...
    if (Array.isArray(relevantDocs)) {
      relevantDocs.forEach((doc: any, index: number) => {
        ...
        console.log(`Name: ${doc.metadata?.name || 'Unknown'}`);
        console.log(`Price: ₦${doc.metadata?.price || 'Unknown'}`);
...
Always be helpful, accurate, and honest about what you know. If someone asks about making a dish, suggest the specific products you have available rather than generic recipes.`;
    ...
    console.log("\n✅ Detailed jollof rice query test completed!");
